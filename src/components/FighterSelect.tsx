...
export default function FighterSelect({ sel, setSel, onStart, onBack }: { sel: Sel; setSel: (s: Sel) => void; onStart: () => void; onBack: () => void }) {
  return (
    <div className="mx-auto max-w-4xl p-4">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4">{FIGHTERS.map(d => <FighterCard key={d.id} d={d} full active={sel.p1 === d.id} onClick={() => setSel({ ...sel, p1: d.id })} />)}</div>
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4">{FIGHTERS.map(d => <FighterCard key={d.id} d={d} active={sel.p2 === d.id} onClick={() => setSel({ ...sel, p2: d.id })} />)}</div>
